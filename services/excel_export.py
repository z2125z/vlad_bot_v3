...
class ExcelExporter:
    ...
    def _export_users(self, writer):
        ...
        for user in users:
            users_data.append({
                ...
                'Last Activity': user.last_activity.strftime('%Y-%m-%d %H:%M') if user.last_activity else '',
                'Days Since Join': (datetime.utcnow() - user.joined_at).days
            })
    ...
    def _export_mailings(self, writer):
        ...
        for mailing in mailings:
            ...
            mailings_data.append({
                ...
                'Created': mailing.created_at.strftime('%Y-%m-%d %H:%M'),
                'Updated': mailing.updated_at.strftime('%Y-%m-%d %H:%M') if mailing.updated_at else '',
                'Text Length': len(mailing.message_text or ''),
                'Has Media': 'Да' if mailing.media_file_id else 'Нет',
                'Buttons Count': len(mailing.buttons or []),
    ...
    def _export_mailing_stats(self, writer):
        ...
        for mailing in mailings:
            ...
            stats_data.append({
                ...
                'Delivered': stats['delivered'],
                'Read': stats['read'],
                'Delivery Rate': f"{(stats['delivered']/stats['total_sent']*100) if stats['total_sent'] > 0 else 0:.1f}%",
                'Read Rate': f"{(stats['read']/stats['total_sent']*100) if stats['total_sent'] > 0 else 0:.1f}%",
                'Success Rate': f"{stats['success_rate']:.1f}%",
                'Created': mailing.created_at.strftime('%Y-%m-%d')
            })
    ...
    def _export_user_activity(self, writer):
        ...
        for user in users:
            days_since_join = (datetime.utcnow() - user.joined_at).days
            days_since_activity = (datetime.utcnow() - user.last_activity).days if user.last_activity else 999
